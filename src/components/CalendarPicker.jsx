...
// markedDates: Set<"YYYY-MM-DD"> — days with appointments (blue dot)
// offDays:     Set<"YYYY-MM-DD"> — off/unavailable days (red tint)
export default function CalendarPicker({ value, onChange, markedDates, offDays }) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 14, padding: '12px 10px', boxShadow: '0 1px 6px rgba(0,0,0,0.06)', marginBottom: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <button onClick={prev} style={{ width: 38, height: 38, borderRadius: 10, background: '#F3F4F6', fontSize: 20, color: '#374151', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>‹</button>
        <span style={{ fontWeight: 800, fontSize: 16, color: '#111827' }}>{MÅNEDER[viewMonth]} {viewYear}</span>
        <button onClick={next} style={{ width: 38, height: 38, borderRadius: 10, background: '#F3F4F6', fontSize: 20, color: '#374151', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>›</button>
      </div>
        ...
        {cells.map((day, i) => {
          ...
          return (
            <button key={day} onClick={() => onChange(d)} style={{
              ...
              background: isSel ? '#2563EB' : isOff ? '#FFF1F2' : isToday ? '#DBEAFE' : 'transparent',
              ...
              {isMarked && (
                <div style={{ width: 4, height: 4, borderRadius: '50%', background: isSel ? 'rgba(255,255,255,0.8)' : isOff ? '#EF4444' : '#2563EB', flexShrink: 0 }} />
              )}
            </button>
          );
        })}

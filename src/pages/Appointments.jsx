...
import { getAppointments, saveAppointments, getCustomers, formatDuration, formatRecurrence, nextOccurrence, getApptColor } from '../storage.js';
            ...
            {sec.data.map(a => {
              ...
              return (
                <div key={a.id} onClick={() => navigate(`/aftaler/${a.id}/rediger`)}
                  ...
                  {/* Dato-boks */}
                  {(() => { const col = isPast ? '#9CA3AF' : getApptColor(a); return (
                  <div style={{ width: 48, height: 52, borderRadius: 10, background: isPast ? '#F3F4F6' : `${col}18`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexShrink: 0, borderLeft: `3px solid ${col}` }}>
                    <span style={{ fontSize: 20, fontWeight: 800, color: col, lineHeight: 1 }}>{d.getDate()}</span>
